...
def extract_long_words(long_words)
  new_long_word = []

  long_words.each {|long_word| new_long_word.push(long_word) if long_word.length > 7 }
...
def pastas_and_sauces (pastas, sauces)
  ...
  pastas.each do |pasta|
    sauces.each do |sauce|
      new_pasta_sauce.push("#{pasta.capitalize} with #{sauce.capitalize} sauce")
    end
  end
  return new_pasta_sauce
end
